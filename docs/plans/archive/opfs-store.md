...
assert(
  (await s.readFile({ path: "Notes/hello.md", encoding: "utf8" })).data ===
    "Hi",
  "utf8 roundtrip",
);
...
assert(
  list.files.length === 1 && list.files[0].name === "hello.md",
  "readdir leaf name",
);
...
assert(
  (await s.readFile({ path: "bin.dat" })).data === bin,
...
assert(
  (await s.readFile({ path: "A/B/C/deep.md", encoding: "utf8" })).data === "x",
  "auto-mkdir parents on write",
);
...
assert(
  (await s.readFile({ path: "Notes/renamed.md", encoding: "utf8" })).data ===
    "Hi",
  "rename dest exists",
);
let gone = false;
try {
  await s.readFile({ path: "Notes/hello.md", encoding: "utf8" });
} catch (e) {
  gone = e.code === "ENOENT" || true;
}
...
assert(
  (await s.readFile({ path: "Notes/copy.md", encoding: "utf8" })).data === "Hi",
  "copy",
);
...
assert(
  ...
  "entries MUST be flat (no children prop)",
);
...
assert(
  (await s.readFile({ path: "log.bin" })).data === btoa("AAABBB"),
  "appendFile = old ⧺ new (exact)",
);
// appendFile ‏על ‏קובץ ‏חסר = ‏יצירה (‏כולל ‏תיקיות-אב)
await s.appendFile({ path: "D/E/new.bin", data: btoa("Z") });
assert(
  (await s.readFile({ path: "D/E/new.bin" })).data === btoa("Z"),
  "appendFile creates missing file + parents",
);

// 12. getUri — blob URL ‏שנטען ‏ומחזיר ‏את ‏התוכן
const { uri } = await s.getUri({ path: "Notes/renamed.md" });
assert(
  typeof uri === "string" && uri.startsWith("blob:"),
  "getUri returns blob: URL",
);
assert(
  (await (await fetch(uri)).text()) === "Hi",
  "getUri blob resolves to file content",
);
```
